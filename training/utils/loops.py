...
def format_losses(total_loss: float, losses: torch.Tensor) -> str:
    terms = ", ".join(
        f"{name}={value:.3g}"
        for name, value in zip(CombinedObjective.LOSS_NAMES, losses.tolist())
    )
...
def train_loop(
    ...
    for batch_idx, (content_images, style_images) in enumerate(loop):
        loop.set_description_str(
            f"Train: Batch {batch_idx + 1}/{len(train_loader)} [Epoch {epoch + 1}/{num_epochs}]"
        )
        ...
        with torch.autocast(
            device_type=device.type, dtype=torch.bfloat16, enabled=mixed_precision
        ):
            stylized_images, target_features = model.transfer(
        ...
        loop.set_postfix_str(
            format_losses(total_loss / (batch_idx + 1), loss_sums / (batch_idx + 1))
        )
...
def val_loop(
    ...
    with torch.no_grad():
        for batch_idx, (content_images, style_images) in enumerate(loop):
            loop.set_description_str(
                f"Val: Batch {batch_idx + 1}/{len(val_loader)} [Epoch {epoch + 1}/{num_epochs}]"
            )
