...
class StyleTransferDataset(Dataset):
    def __init__(
        ...
        self.content_images = sorted(
            path
            for path in self.content_dir.glob("*")
            if path.suffix.lower() in IMAGE_EXTENSIONS
        )
        self.style_images = sorted(
            path
            for path in self.style_dir.glob("*")
            if path.suffix.lower() in IMAGE_EXTENSIONS
        )
        ...
        if self.max_images is not None:
            ...
            self.content_images = sorted(
                rng.sample(
                    self.content_images, min(self.max_images, len(self.content_images))
                )
            )
            self.style_images = sorted(
                rng.sample(
                    self.style_images, min(self.max_images, len(self.style_images))
                )
            )
