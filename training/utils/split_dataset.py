...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the dataset directory. Images anywhere below it are moved into train/ and val/ subdirectories.",
    )
    parser.add_argument(
        "--train-ratio",
        type=float,
        default=0.8,
        help="Ratio of images to use for training (between 0 and 1, exclusive).",
    )
    parser.add_argument(
    ...
    if not 0 < args.train_ratio < 1:
        parser.error(
            f"--train-ratio must be between 0 and 1, exclusive, got {args.train_ratio}"
        )

    return args


def get_all_nested_image_paths(root_dir: Path) -> list[Path]:
...
def split_dataset(
    filenames: list[str], train_ratio: float, seed: int
    ...
    if not train_filenames or not val_filenames:
        raise ValueError(
            f"Splitting {len(filenames)} images with train ratio {train_ratio} leaves train or val empty"
        )
...
def plan_moves(
    ...
    moves = []
    for split_dir, filenames in (
        (root_dir / "train", train_filenames),
        (root_dir / "val", val_filenames),
    ):
    ...
    if len(set(destinations)) != len(destinations):
        raise ValueError(
            "Two images would be moved to the same destination; nothing has been moved"
        )
...
def move_images(root_dir: Path, moves: list[tuple[Path, Path]], output_file: Path):
    ...
    with open(output_file, "w", newline="") as f:
        ...
        for original_path, new_path in moves:
            ...
            writer.writerow(
                [original_path.relative_to(root_dir), new_path.relative_to(root_dir)]
            )
...
def main():
    ...
    existing_splits = [
        name for name in ("train", "val") if (dataset_rootdir / name).exists()
    ]
    if existing_splits:
        sys.exit(
            f"{dataset_rootdir} already contains {' and '.join(existing_splits)}/; refusing to split again"
        )

    all_image_paths = get_all_nested_image_paths(dataset_rootdir)
    groups = group_by_filename(all_image_paths)
    train_filenames, val_filenames = split_dataset(
        list(groups), args.train_ratio, args.seed
    )
    ...
    print(
        f"Moved {num_train} images to train/ and {len(moves) - num_train} to val/. Record written to {output_file}"
    )
